...
export default function PrintCertificate() {
  ...
  // 이름 길이에 따라 '성명' 위치 조정 (7자 초과 시 기록 아래로 이동)
  ...
  return (
    <div
      ...
      className="relative overflow-hidden w-[595px] h-[842px] bg-white px-[100px] py-[120px] text-center flex flex-col font-chosungs text-black mx-auto print:w-[595px] print:h-[842px]"
    >
      {/* 중앙 워터마크 (배경 로고) */}
      <div
        aria-hidden="true"
      ...
      {/* 실제 내용: 본문 + 푸터를 분리하여 푸터를 항상 하단 고정 */}
      <div className="relative z-10 flex h-full flex-col">
        {/* 본문 */}
        <div>
          <div className="text-left text-xl mb-2">제{data?.cert_data?.cert_num}호</div>

          <h1 className={`text-8xl font-bold tracking-wider ${isLongName ? 'mb-16' : 'mb-20'}`}>
            상&nbsp;&nbsp;&nbsp;장
          </h1>
            ...
            <div className="flex flex-col items-start w-2/5 gap-3">
              ...
              {!isLongName && (
                <div className="flex items-start w-full">
                  ...
                  {/* line-height 고정(두 경우 동일하게) */}
                  <span className="flex-1 text-left whitespace-normal break-keep leading-[2rem]">
                    {playerName}
                  </span>
                </div>
              )}
          ...
          {isLongName && (
            <div className="mt-2 w-full text-2xl">
              {/* 라벨 고정폭(4rem) + 값 가변폭: 줄바꿈 들여쓰기 정렬 */}
              <div className="grid grid-cols-[4rem_1fr] items-start gap-1">
                <div className="text-left">성명 :</div>
                {/* line-height 고정(두 경우 동일하게) */}
                <div className="text-left whitespace-nowrap break-keep leading-[2rem]">
                  {playerName}
                </div>
              </div>
            </div>
        ...
        {/* 푸터: 항상 아래로 밀착 (mt-auto) */}
        <div className="mt-auto text-3xl leading-4 mb-20">
          ...
          <div className="mt-2 relative inline-block">
            <Image
              src={dojang}
              alt="도장"
              className="absolute right-[-90px] top-[-35px] z-0"
            ...
            <p className="inline-block font-bold text-3xl relative z-10">
              ...
              <span className="text-4xl font-bold">
                {data?.cert_data?.president?.split('').map((ch, i) => (
                  <span key={i}>
                    {ch}
                    {i < (data?.cert_data?.president?.length ?? 0) - 1 ? '\u00A0\u00A0\u00A0' : ''}
                  </span>
                ))}
              </span>
            </p>
